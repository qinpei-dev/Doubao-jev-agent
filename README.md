...
The deterministic demo reads the actual `README.md`, writes `output/summary.md` inside the configured sandbox, and prints a structured trace. It uses the local JEV mock by default and stays offline; pass `--real-jev` to use the configured `JEV_API_KEY`. If the output already exists, the action waits for review; pass `--approve-existing` only when you intend to overwrite it. The tool execution itself is real.
